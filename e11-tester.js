...
const { getBaseUrl, getUTorId } = require("../E11/scripts.js");
  ...
  try {
    ...
    if (match && match.length > 1) {
      ...
      if (match[1].trim() === expectedContent) {
        ...
        console.log(
          "Your deployment is live, and it’s amazing to see how far you’ve come! Keep pushing forward.\nWow, you’re building the future, one line of code at a time. Great job!"
        );
        process.exit(0);
      } else {
        console.log(
          '❌ Auto-tester failed: The content of the "auto-tester" div does not match the expected output.'
        );
        console.log(`Received: ${match[1].trim()}`);
        console.log(`Expected: ${expectedContent}`);
        console.log(
          "Don’t be discouraged -- every bug is just another step on the path to mastery."
        );
      }
    } else {
      console.log(
